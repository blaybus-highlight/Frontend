...
export const ProductInfoForm = ({ formData, onFormChange }: ProductInfoFormProps) => {
  return (
      ...
      <div className="flex flex-col ml-4 gap-4 w-full pr-4">
        {formConfig.map((config, index) => {
            ...
            return (
              <div key={index} className="flex flex-row items-center w-full gap-4">
                {config.fields.map(field => {
                  ...
                  return <Component key={field.name} {...field} value={formData[field.name] || ''} onChange={onFormChange} />;
                })}
              </div>
            );
          }
          ...
          if (config.type === 'full') {
            ...
            return <Component key={config.field.name} {...config.field} value={formData[config.field.name] || ''} onChange={onFormChange} />;
          }
          return null;
        })}
      </div>
